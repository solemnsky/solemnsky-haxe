...
Main.__name__ = ["Main"];
Main.main = function() {
	var control1 = control_demo_AllDemo.run();
...
Manager.prototype = $extend(kha_Game.prototype,{
	...
	,render: function(frame) {
		...
		if(this.profileTicker > 100) {
			this.profileTicker = 0;
			this.ctrl.profiling(new control_Profile(this.bufferOn,this.renderOn,this.renderOff,this.primCount,this.tickOn,this.tickOff));
		}
		var now = haxe_Timer.stamp();
...
$hxClasses["control.Control"] = control_Control;
control_Control.__name__ = ["control","Control"];
control_Control.prototype = {
	__class__: control_Control
};
var control_EmptyControl = function() {
};
$hxClasses["control.EmptyControl"] = control_EmptyControl;
control_EmptyControl.__name__ = ["control","EmptyControl"];
control_EmptyControl.__interfaces__ = [control_Control];
control_EmptyControl.prototype = {
	init: function(_) {
	}
	,tick: function(delta) {
	}
	...
	,profiling: function(data) {
		haxe_Log.trace(data.print(),{ fileName : "Control.hx", lineNumber : 79, className : "control.EmptyControl", methodName : "profiling"});
	}
	,handle: function(e) {
	}
	,conclude: function() {
...
var control_Noise = $hxClasses["control.Noise"] = { __ename__ : ["control","Noise"], __constructs__ : ["Noise"] };
control_Noise.Noise = ["Noise",0];
control_Noise.Noise.toString = $estr;
control_Noise.Noise.__enum__ = control_Noise;
var control_Profile = function(bufferOn,renderOn,renderOff,primCount,tickOn,tickOff) {
	this.bufferOn = control_Profile.dataFromArray(bufferOn);
	this.renderOn = control_Profile.dataFromArray(renderOn);
	this.renderOff = control_Profile.dataFromArray(renderOff);
	this.primCount = control_Profile.dataFromArray(primCount);
	this.tickOn = control_Profile.dataFromArray(tickOn);
	this.tickOff = control_Profile.dataFromArray(tickOff);
};
$hxClasses["control.Profile"] = control_Profile;
control_Profile.__name__ = ["control","Profile"];
control_Profile.dataFromArray = function(a) {
	var sum = 0;
	var min = a[0];
	var max = a[0];
	var _g = 0;
...
control_Profile.printInterval = function(i) {
	return i.min + "ms-" + i.max + "ms~" + i.average + "ms";
};
control_Profile.printValue = function(i) {
	return i.min + "-" + i.max + "~" + i.average;
};
control_Profile.prototype = {
	print: function() {
		return "" + "buff:" + control_Profile.printInterval(this.bufferOn) + "/" + "rndr:" + control_Profile.printInterval(this.renderOn) + "/" + "rslp:" + control_Profile.printInterval(this.renderOff) + "/" + "\n" + "prim:" + control_Profile.printValue(this.primCount) + "/" + "tick:" + control_Profile.printInterval(this.tickOn) + "/" + "tslp:" + control_Profile.printInterval(this.tickOff) + "/";
	}
	,__class__: control_Profile
};
var control_TextAlign = $hxClasses["control.TextAlign"] = { __ename__ : ["control","TextAlign"], __constructs__ : ["CenterText","LeftText","RightText"] };
control_TextAlign.CenterText = ["CenterText",0];
control_TextAlign.CenterText.toString = $estr;
control_TextAlign.CenterText.__enum__ = control_TextAlign;
control_TextAlign.LeftText = ["LeftText",1];
control_TextAlign.LeftText.toString = $estr;
control_TextAlign.LeftText.__enum__ = control_TextAlign;
control_TextAlign.RightText = ["RightText",2];
control_TextAlign.RightText.toString = $estr;
control_TextAlign.RightText.__enum__ = control_TextAlign;
var control_DrawPrim = $hxClasses["control.DrawPrim"] = { __ename__ : ["control","DrawPrim"], __constructs__ : ["SetColor","SetFont","DrawCircle","DrawRect","DrawText","DrawImage"] };
control_DrawPrim.SetColor = function(r,g,b,a) { var $x = ["SetColor",0,r,g,b,a]; $x.__enum__ = control_DrawPrim; $x.toString = $estr; return $x; };
control_DrawPrim.SetFont = function(name,size) { var $x = ["SetFont",1,name,size]; $x.__enum__ = control_DrawPrim; $x.toString = $estr; return $x; };
control_DrawPrim.DrawCircle = function(pos,radius) { var $x = ["DrawCircle",2,pos,radius]; $x.__enum__ = control_DrawPrim; $x.toString = $estr; return $x; };
control_DrawPrim.DrawRect = function(topLeft,bottomRight) { var $x = ["DrawRect",3,topLeft,bottomRight]; $x.__enum__ = control_DrawPrim; $x.toString = $estr; return $x; };
control_DrawPrim.DrawText = function(pos,align,text) { var $x = ["DrawText",4,pos,align,text]; $x.__enum__ = control_DrawPrim; $x.toString = $estr; return $x; };
control_DrawPrim.DrawImage = function(pos,image) { var $x = ["DrawImage",5,pos,image]; $x.__enum__ = control_DrawPrim; $x.toString = $estr; return $x; };
var control_Scene = function() {
	this.alpha = 1;
	this.trans = new math_Transform(1,0,0,0,1,0,0,0,1);
	this.children = [];
	this.prims = [];
};
$hxClasses["control.Scene"] = control_Scene;
control_Scene.__name__ = ["control","Scene"];
...
control_demo_SelectionScreen.__interfaces__ = [control_Control];
control_demo_SelectionScreen.renderText = function() {
	var scene = new control_Scene();
	var text_0 = "Welcome to the solemnsky control technical demo.";
	var text_1 = "What you are seeing is the the control object control.AllDemo.run()";
	var text_2 = "compiled through one of our export media.";
	var text_3 = "It demonstrates of several features that should be achieved in a ";
	var text_4 = " technically sound export media. ";
	var text_5 = "Press 1, 2, or 3 on your keyboard to continue.";
	scene.prims = [control_DrawPrim.SetColor(0,0,0,255),control_DrawPrim.SetFont("Arial",14),control_DrawPrim.DrawText(new math_Vector(266.666666666666686,0),control_TextAlign.CenterText,text_0),control_DrawPrim.DrawText(new math_Vector(0,25),control_TextAlign.LeftText,text_1),control_DrawPrim.DrawText(new math_Vector(0,40),control_TextAlign.LeftText,text_2),control_DrawPrim.DrawText(new math_Vector(0,65),control_TextAlign.LeftText,text_3),control_DrawPrim.DrawText(new math_Vector(0,80),control_TextAlign.LeftText,text_4),control_DrawPrim.DrawText(new math_Vector(266.666666666666686,105),control_TextAlign.CenterText,text_5)];
	scene.trans = new math_Transform(1,0,0,0,1,5,0,0,1).multmat(new math_Transform(3,0,0,0,3,0,0,0,1));
	return scene;
};
...
control_demo_AllDemo.run = function() {
	return new control_demo_SelectionScreen();
};
...
math_Vector.zero = new math_Vector(0,0);
Main.main();
